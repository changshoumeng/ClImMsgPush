...
class TcpFrontWorker(TcpAcceptor):    
    ...
    def makeTask(self,packet):
        self.taskId += 1
        taskName="t{0}_{1}".format(self.sessionId,self.taskId )
        #dump_log( "TcpFrontWorker putTask {0}".format(taskName)  )
        t=HttpTask(taskName,packet)
        return  t
             
    
    ########################
    #override method      unpackFromBuffer
    ########################    
    def unpackFromBuffer(self,leftBuffer):
        ...
        if net_head.m_uPacketSize > 4096 or   net_head.m_uPacketSize <16:
            info="unpackFromBuffer packetSize:{0} error:".format(net_head.m_uPacketSize)
            dump_log(info)
            return -1
    ...
    def sendPacket(self,packet):
        data=packet.pack()
        self.sendData(data)
        info="sendPacket->cmd:{0} size:{1}".format(packet.m_uMainCmd,packet.m_uPacketSize)
        dump_log(info)
